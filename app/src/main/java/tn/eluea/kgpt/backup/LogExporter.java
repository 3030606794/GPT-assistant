...
public class LogExporter {
    ...
    public ExportResult exportLogs(Uri uri) {
        ...
        try (OutputStream os = context.getContentResolver().openOutputStream(uri);
            ...
            // Add internal AI diagnostics ring buffer (cross-process, survives incomplete logcat capture)
            addAiDiagnostics(zos);
    ...
    private void addAiDiagnostics(ZipOutputStream zos) throws IOException {
        String content;
        try {
            content = AiDiagnostics.readAll();
        } catch (Throwable t) {
            content = "(failed to read ai diagnostics)\n" + String.valueOf(t);
        }
        if (content == null || content.trim().isEmpty()) {
            content = "(empty)";
        }
        addZipEntry(zos, "ai_diagnostics.txt", content);

        // Add last-request slices for easier debugging of streaming / interrupts
        try {
            String meta = AiDiagnostics.readLastRequestMeta();
            if (meta == null || meta.trim().isEmpty()) meta = "(empty)";
            addZipEntry(zos, "last_request_meta.txt", meta);
        } catch (Throwable ignored) {
            addZipEntry(zos, "last_request_meta.txt", "(failed)");
        }

        try {
            String raw = AiDiagnostics.readLastRaw();
            if (raw == null || raw.trim().isEmpty()) raw = "(empty)";
            addZipEntry(zos, "last_response_raw.txt", raw);
        } catch (Throwable ignored) {
            addZipEntry(zos, "last_response_raw.txt", "(failed)");
        }

        try {
            String intr = AiDiagnostics.readLastInterrupt();
            if (intr == null || intr.trim().isEmpty()) intr = "(empty)";
            addZipEntry(zos, "last_interrupt_info.txt", intr);
        } catch (Throwable ignored) {
            addZipEntry(zos, "last_interrupt_info.txt", "(failed)");
        }
