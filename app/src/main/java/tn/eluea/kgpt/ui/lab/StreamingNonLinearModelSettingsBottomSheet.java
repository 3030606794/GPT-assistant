...
public final class StreamingNonLinearModelSettingsBottomSheet {
    ...
    public static Runnable bindForScreen(@NonNull Context context,
        ...
        View panelPerlin = content.findViewById(R.id.panel_perlin);
        View panelPid = content.findViewById(R.id.panel_pid);
        View panelLogistic = content.findViewById(R.id.panel_logistic);
        View panelRetro = content.findViewById(R.id.panel_retro);
        ...
        View btnResetPerlin = content.findViewById(R.id.btn_reset_perlin);
        View btnResetPid = content.findViewById(R.id.btn_reset_pid);
        View btnResetLogistic = content.findViewById(R.id.btn_reset_logistic);
        View btnResetRetro = content.findViewById(R.id.btn_reset_retro);
        ...
        // Perlin
        Slider sPerlinBaseSpeed = content.findViewById(R.id.slider_perlin_base_speed);
        TextView tvPerlinBaseSpeed = content.findViewById(R.id.tv_perlin_base_speed_value);
        Slider sPerlinOctaves = content.findViewById(R.id.slider_perlin_octaves);
        TextView tvPerlinOctaves = content.findViewById(R.id.tv_perlin_octaves_value);
        Slider sPerlinFrequency = content.findViewById(R.id.slider_perlin_frequency);
        TextView tvPerlinFrequency = content.findViewById(R.id.tv_perlin_frequency_value);

        // PID
        Slider sPidTargetSpeed = content.findViewById(R.id.slider_pid_target_speed);
        TextView tvPidTargetSpeed = content.findViewById(R.id.tv_pid_target_speed_value);
        Slider sPidP = content.findViewById(R.id.slider_pid_p);
        TextView tvPidP = content.findViewById(R.id.tv_pid_p_value);
        Slider sPidD = content.findViewById(R.id.slider_pid_d);
        TextView tvPidD = content.findViewById(R.id.tv_pid_d_value);

        // Logistic
        Slider sLogiInitial = content.findViewById(R.id.slider_logistic_initial);
        TextView tvLogiInitial = content.findViewById(R.id.tv_logistic_initial_value);
        Slider sLogiDecay = content.findViewById(R.id.slider_logistic_decay);
        TextView tvLogiDecay = content.findViewById(R.id.tv_logistic_decay_value);
        Slider sLogiRecovery = content.findViewById(R.id.slider_logistic_recovery);
        TextView tvLogiRecovery = content.findViewById(R.id.tv_logistic_recovery_value);

        // Retro
        Slider sRetroKeySpeed = content.findViewById(R.id.slider_retro_key_speed);
        TextView tvRetroKeySpeed = content.findViewById(R.id.tv_retro_key_speed_value);
        Slider sRetroCrDelay = content.findViewById(R.id.slider_retro_cr_delay);
        TextView tvRetroCrDelay = content.findViewById(R.id.tv_retro_cr_delay_value);
        Slider sRetroPuncDrag = content.findViewById(R.id.slider_retro_punc_drag);
        TextView tvRetroPuncDrag = content.findViewById(R.id.tv_retro_punc_drag_value);
...
if (itemModelTrigger != null) {
    ...
    final Runnable showModelDialog = () -> {
        ...
        AlertDialog dialog = new MaterialAlertDialogBuilder(context)
                ...
                .setPositiveButton(android.R.string.ok, (d, which) -> {
                    ...
                    updatePanels(selected, panelLc, panelExp, panelSine, panelDamp, panelSquare, panelMarkov, panelPerlin, panelPid, panelLogistic, panelRetro);
                    refreshAll.run();
                })
                .create();
        dialog.show();
    ...
    updatePanels(currentModel, panelLc, panelExp, panelSine, panelDamp, panelSquare, panelMarkov, panelPerlin, panelPid, panelLogistic, panelRetro);
} else {
    // If the trigger view is missing, still keep panel visibility coherent.
    updatePanels(selectedModelHolder[0], panelLc, panelExp, panelSine, panelDamp, panelSquare, panelMarkov, panelPerlin, panelPid, panelLogistic, panelRetro);
}
        ...
        // ---- v16+ geek model sliders ----

        if (sPerlinBaseSpeed != null && tvPerlinBaseSpeed != null) {
            float raw = (float) sp.getNlPerlinBaseSpeed();
            float v = snapToSlider(sPerlinBaseSpeed, raw);
            if (!nearlyEqual(v, raw)) sp.setNlPerlinBaseSpeed(v);
            sPerlinBaseSpeed.setValue(v);
            tvPerlinBaseSpeed.setText(formatMultiplier(v));
            sPerlinBaseSpeed.addOnChangeListener((slider, value, fromUser) -> {
                tvPerlinBaseSpeed.setText(formatMultiplier(value));
                refreshAll.run();
            });
            sPerlinBaseSpeed.addOnSliderTouchListener(new SimpleTouchListener(() -> {
                float snapped = snapToSlider(sPerlinBaseSpeed, sPerlinBaseSpeed.getValue());
                if (!nearlyEqual(snapped, sPerlinBaseSpeed.getValue())) sPerlinBaseSpeed.setValue(snapped);
                sp.setNlPerlinBaseSpeed(snapped);
                refreshAll.run();
            }));
        }

        if (sPerlinOctaves != null && tvPerlinOctaves != null) {
            float raw = sp.getNlPerlinOctaves();
            float v = snapToSlider(sPerlinOctaves, raw);
            if (!nearlyEqual(v, raw)) sp.setNlPerlinOctaves(Math.round(v));
            sPerlinOctaves.setValue(v);
            tvPerlinOctaves.setText(String.valueOf(Math.round(v)));
            sPerlinOctaves.addOnChangeListener((slider, value, fromUser) -> {
                tvPerlinOctaves.setText(String.valueOf(Math.round(value)));
                refreshAll.run();
            });
            sPerlinOctaves.addOnSliderTouchListener(new SimpleTouchListener(() -> {
                float snapped = snapToSlider(sPerlinOctaves, sPerlinOctaves.getValue());
                if (!nearlyEqual(snapped, sPerlinOctaves.getValue())) sPerlinOctaves.setValue(snapped);
                sp.setNlPerlinOctaves(Math.round(snapped));
                refreshAll.run();
            }));
        }

        if (sPerlinFrequency != null && tvPerlinFrequency != null) {
            float raw = (float) sp.getNlPerlinFrequency();
            float v = snapToSlider(sPerlinFrequency, raw);
            if (!nearlyEqual(v, raw)) sp.setNlPerlinFrequency(v);
            sPerlinFrequency.setValue(v);
            tvPerlinFrequency.setText(formatDouble(v, 2));
            sPerlinFrequency.addOnChangeListener((slider, value, fromUser) -> {
                tvPerlinFrequency.setText(formatDouble(value, 2));
                refreshAll.run();
            });
            sPerlinFrequency.addOnSliderTouchListener(new SimpleTouchListener(() -> {
                float snapped = snapToSlider(sPerlinFrequency, sPerlinFrequency.getValue());
                if (!nearlyEqual(snapped, sPerlinFrequency.getValue())) sPerlinFrequency.setValue(snapped);
                sp.setNlPerlinFrequency(snapped);
                refreshAll.run();
            }));
        }

        if (sPidTargetSpeed != null && tvPidTargetSpeed != null) {
            float raw = (float) sp.getNlPidTargetSpeed();
            float v = snapToSlider(sPidTargetSpeed, raw);
            if (!nearlyEqual(v, raw)) sp.setNlPidTargetSpeed(v);
            sPidTargetSpeed.setValue(v);
            tvPidTargetSpeed.setText(formatMultiplier(v));
            sPidTargetSpeed.addOnChangeListener((slider, value, fromUser) -> {
                tvPidTargetSpeed.setText(formatMultiplier(value));
                refreshAll.run();
            });
            sPidTargetSpeed.addOnSliderTouchListener(new SimpleTouchListener(() -> {
                float snapped = snapToSlider(sPidTargetSpeed, sPidTargetSpeed.getValue());
                if (!nearlyEqual(snapped, sPidTargetSpeed.getValue())) sPidTargetSpeed.setValue(snapped);
                sp.setNlPidTargetSpeed(snapped);
                refreshAll.run();
            }));
        }

        if (sPidP != null && tvPidP != null) {
            float raw = (float) sp.getNlPidP();
            float v = snapToSlider(sPidP, raw);
            if (!nearlyEqual(v, raw)) sp.setNlPidP(v);
            sPidP.setValue(v);
            tvPidP.setText(formatDouble(v, 2));
            sPidP.addOnChangeListener((slider, value, fromUser) -> {
                tvPidP.setText(formatDouble(value, 2));
                refreshAll.run();
            });
            sPidP.addOnSliderTouchListener(new SimpleTouchListener(() -> {
                float snapped = snapToSlider(sPidP, sPidP.getValue());
                if (!nearlyEqual(snapped, sPidP.getValue())) sPidP.setValue(snapped);
                sp.setNlPidP(snapped);
                refreshAll.run();
            }));
        }

        if (sPidD != null && tvPidD != null) {
            float raw = (float) sp.getNlPidD();
            float v = snapToSlider(sPidD, raw);
            if (!nearlyEqual(v, raw)) sp.setNlPidD(v);
            sPidD.setValue(v);
            tvPidD.setText(formatDouble(v, 2));
            sPidD.addOnChangeListener((slider, value, fromUser) -> {
                tvPidD.setText(formatDouble(value, 2));
                refreshAll.run();
            });
            sPidD.addOnSliderTouchListener(new SimpleTouchListener(() -> {
                float snapped = snapToSlider(sPidD, sPidD.getValue());
                if (!nearlyEqual(snapped, sPidD.getValue())) sPidD.setValue(snapped);
                sp.setNlPidD(snapped);
                refreshAll.run();
            }));
        }

        if (sLogiInitial != null && tvLogiInitial != null) {
            float raw = sp.getNlLogisticInitialBurstMs();
            float v = snapToSlider(sLogiInitial, raw);
            if (!nearlyEqual(v, raw)) sp.setNlLogisticInitialBurstMs(Math.round(v));
            sLogiInitial.setValue(v);
            tvLogiInitial.setText(formatMs(Math.round(v)));
            sLogiInitial.addOnChangeListener((slider, value, fromUser) -> {
                tvLogiInitial.setText(formatMs(Math.round(value)));
                refreshAll.run();
            });
            sLogiInitial.addOnSliderTouchListener(new SimpleTouchListener(() -> {
                float snapped = snapToSlider(sLogiInitial, sLogiInitial.getValue());
                if (!nearlyEqual(snapped, sLogiInitial.getValue())) sLogiInitial.setValue(snapped);
                sp.setNlLogisticInitialBurstMs(Math.round(snapped));
                refreshAll.run();
            }));
        }

        if (sLogiDecay != null && tvLogiDecay != null) {
            float raw = (float) sp.getNlLogisticDecayHalfLife();
            float v = snapToSlider(sLogiDecay, raw);
            if (!nearlyEqual(v, raw)) sp.setNlLogisticDecayHalfLife(v);
            sLogiDecay.setValue(v);
            tvLogiDecay.setText(formatDouble(v, 0));
            sLogiDecay.addOnChangeListener((slider, value, fromUser) -> {
                tvLogiDecay.setText(formatDouble(value, 0));
                refreshAll.run();
            });
            sLogiDecay.addOnSliderTouchListener(new SimpleTouchListener(() -> {
                float snapped = snapToSlider(sLogiDecay, sLogiDecay.getValue());
                if (!nearlyEqual(snapped, sLogiDecay.getValue())) sLogiDecay.setValue(snapped);
                sp.setNlLogisticDecayHalfLife(snapped);
                refreshAll.run();
            }));
        }

        if (sLogiRecovery != null && tvLogiRecovery != null) {
            float raw = (float) sp.getNlLogisticRecovery();
            float v = snapToSlider(sLogiRecovery, raw);
            if (!nearlyEqual(v, raw)) sp.setNlLogisticRecovery(v);
            sLogiRecovery.setValue(v);
            tvLogiRecovery.setText(formatDouble(v, 2));
            sLogiRecovery.addOnChangeListener((slider, value, fromUser) -> {
                tvLogiRecovery.setText(formatDouble(value, 2));
                refreshAll.run();
            });
            sLogiRecovery.addOnSliderTouchListener(new SimpleTouchListener(() -> {
                float snapped = snapToSlider(sLogiRecovery, sLogiRecovery.getValue());
                if (!nearlyEqual(snapped, sLogiRecovery.getValue())) sLogiRecovery.setValue(snapped);
                sp.setNlLogisticRecovery(snapped);
                refreshAll.run();
            }));
        }

        if (sRetroKeySpeed != null && tvRetroKeySpeed != null) {
            float raw = sp.getNlRetroKeySpeedMs();
            float v = snapToSlider(sRetroKeySpeed, raw);
            if (!nearlyEqual(v, raw)) sp.setNlRetroKeySpeedMs(Math.round(v));
            sRetroKeySpeed.setValue(v);
            tvRetroKeySpeed.setText(formatMs(Math.round(v)));
            sRetroKeySpeed.addOnChangeListener((slider, value, fromUser) -> {
                tvRetroKeySpeed.setText(formatMs(Math.round(value)));
                refreshAll.run();
            });
            sRetroKeySpeed.addOnSliderTouchListener(new SimpleTouchListener(() -> {
                float snapped = snapToSlider(sRetroKeySpeed, sRetroKeySpeed.getValue());
                if (!nearlyEqual(snapped, sRetroKeySpeed.getValue())) sRetroKeySpeed.setValue(snapped);
                sp.setNlRetroKeySpeedMs(Math.round(snapped));
                refreshAll.run();
            }));
        }

        if (sRetroCrDelay != null && tvRetroCrDelay != null) {
            float raw = sp.getNlRetroCrDelayMs();
            float v = snapToSlider(sRetroCrDelay, raw);
            if (!nearlyEqual(v, raw)) sp.setNlRetroCrDelayMs(Math.round(v));
            sRetroCrDelay.setValue(v);
            tvRetroCrDelay.setText(formatMs(Math.round(v)));
            sRetroCrDelay.addOnChangeListener((slider, value, fromUser) -> {
                tvRetroCrDelay.setText(formatMs(Math.round(value)));
                refreshAll.run();
            });
            sRetroCrDelay.addOnSliderTouchListener(new SimpleTouchListener(() -> {
                float snapped = snapToSlider(sRetroCrDelay, sRetroCrDelay.getValue());
                if (!nearlyEqual(snapped, sRetroCrDelay.getValue())) sRetroCrDelay.setValue(snapped);
                sp.setNlRetroCrDelayMs(Math.round(snapped));
                refreshAll.run();
            }));
        }

        if (sRetroPuncDrag != null && tvRetroPuncDrag != null) {
            float raw = sp.getNlRetroPuncDragMs();
            float v = snapToSlider(sRetroPuncDrag, raw);
            if (!nearlyEqual(v, raw)) sp.setNlRetroPuncDragMs(Math.round(v));
            sRetroPuncDrag.setValue(v);
            tvRetroPuncDrag.setText(formatMs(Math.round(v)));
            sRetroPuncDrag.addOnChangeListener((slider, value, fromUser) -> {
                tvRetroPuncDrag.setText(formatMs(Math.round(value)));
                refreshAll.run();
            });
            sRetroPuncDrag.addOnSliderTouchListener(new SimpleTouchListener(() -> {
                float snapped = snapToSlider(sRetroPuncDrag, sRetroPuncDrag.getValue());
                if (!nearlyEqual(snapped, sRetroPuncDrag.getValue())) sRetroPuncDrag.setValue(snapped);
                sp.setNlRetroPuncDragMs(Math.round(snapped));
                refreshAll.run();
            }));
        }
...
if (btnResetPerlin != null) {
    btnResetPerlin.setOnClickListener(v -> {
        float defBase = 1.25f;
        int defOct = 3;
        float defFreq = 1.10f;
        sp.setNlPerlinBaseSpeed(defBase);
        sp.setNlPerlinOctaves(defOct);
        sp.setNlPerlinFrequency(defFreq);
        if (sPerlinBaseSpeed != null) sPerlinBaseSpeed.setValue(snapToSlider(sPerlinBaseSpeed, defBase));
        if (sPerlinOctaves != null) sPerlinOctaves.setValue(snapToSlider(sPerlinOctaves, defOct));
        if (sPerlinFrequency != null) sPerlinFrequency.setValue(snapToSlider(sPerlinFrequency, defFreq));
        refreshAll.run();
    });
}

if (btnResetPid != null) {
    btnResetPid.setOnClickListener(v -> {
        float defTarget = 1.35f;
        float defP = 0.22f;
        float defD = 0.12f;
        sp.setNlPidTargetSpeed(defTarget);
        sp.setNlPidP(defP);
        sp.setNlPidD(defD);
        if (sPidTargetSpeed != null) sPidTargetSpeed.setValue(snapToSlider(sPidTargetSpeed, defTarget));
        if (sPidP != null) sPidP.setValue(snapToSlider(sPidP, defP));
        if (sPidD != null) sPidD.setValue(snapToSlider(sPidD, defD));
        refreshAll.run();
    });
}

if (btnResetLogistic != null) {
    btnResetLogistic.setOnClickListener(v -> {
        int defBurst = 24;
        float defHalf = 40.0f;
        float defRec = 0.35f;
        sp.setNlLogisticInitialBurstMs(defBurst);
        sp.setNlLogisticDecayHalfLife(defHalf);
        sp.setNlLogisticRecovery(defRec);
        if (sLogiInitial != null) sLogiInitial.setValue(snapToSlider(sLogiInitial, defBurst));
        if (sLogiDecay != null) sLogiDecay.setValue(snapToSlider(sLogiDecay, defHalf));
        if (sLogiRecovery != null) sLogiRecovery.setValue(snapToSlider(sLogiRecovery, defRec));
        refreshAll.run();
    });
}

if (btnResetRetro != null) {
    btnResetRetro.setOnClickListener(v -> {
        int defKey = 38;
        int defCr = 240;
        int defDrag = 80;
        sp.setNlRetroKeySpeedMs(defKey);
        sp.setNlRetroCrDelayMs(defCr);
        sp.setNlRetroPuncDragMs(defDrag);
        if (sRetroKeySpeed != null) sRetroKeySpeed.setValue(snapToSlider(sRetroKeySpeed, defKey));
        if (sRetroCrDelay != null) sRetroCrDelay.setValue(snapToSlider(sRetroCrDelay, defCr));
        if (sRetroPuncDrag != null) sRetroPuncDrag.setValue(snapToSlider(sRetroPuncDrag, defDrag));
        refreshAll.run();
    });
}
    ...
    private static void updatePanels(int model,
                                     ...
                                     @Nullable View panelPerlin,
                                     @Nullable View panelPid,
                                     @Nullable View panelLogistic,
                                     @Nullable View panelRetro) {
        ...
        if (panelPerlin != null) panelPerlin.setVisibility(model == SPManager.STREAM_NL_MODEL_PERLIN_NOISE ? View.VISIBLE : View.GONE);
        if (panelPid != null) panelPid.setVisibility(model == SPManager.STREAM_NL_MODEL_PID_CONTROLLER ? View.VISIBLE : View.GONE);
        if (panelLogistic != null) panelLogistic.setVisibility(model == SPManager.STREAM_NL_MODEL_LOGISTIC_FATIGUE ? View.VISIBLE : View.GONE);
        if (panelRetro != null) panelRetro.setVisibility(model == SPManager.STREAM_NL_MODEL_RETRO_TYPEWRITER ? View.VISIBLE : View.GONE);
    }
    ...
    private static void updateModelInfo(@NonNull Context context,
                                        ...
                                        @Nullable Slider sPerlinBaseSpeed,
                                        @Nullable Slider sPerlinOctaves,
                                        @Nullable Slider sPerlinFrequency,
                                        @Nullable Slider sPidTargetSpeed,
                                        @Nullable Slider sPidP,
                                        @Nullable Slider sPidD,
                                        @Nullable Slider sLogiInitial,
                                        @Nullable Slider sLogiDecay,
                                        @Nullable Slider sLogiRecovery,
                                        @Nullable Slider sRetroKeySpeed,
                                        @Nullable Slider sRetroCrDelay,
                                        @Nullable Slider sRetroPuncDrag) {
        if (tvDesc != null) {
    ...
    private static void updatePreview(@NonNull SPManager sp,
        ...
        switch (model) {
            ...
            case SPManager.STREAM_NL_MODEL_PERLIN_NOISE: {
                double base = sp.getNlPerlinBaseSpeed();
                int oct = sp.getNlPerlinOctaves();
                double freq = sp.getNlPerlinFrequency();
                if (sPerlinBaseSpeed != null) base = sPerlinBaseSpeed.getValue();
                if (sPerlinOctaves != null) oct = Math.round(sPerlinOctaves.getValue());
                if (sPerlinFrequency != null) freq = sPerlinFrequency.getValue();
                // Preview: deterministic "Perlin-ish" smooth noise (multi-sine octaves with stable phases).
                // Base is a speed multiplier: higher => lower delay.
                double baseDelay = 95.0 / Math.max(0.10, base);
                ...
                for (int i = 0; i < phase.length; i++) phase[i] = rng.nextDouble() * (2.0 * Math.PI);

                // UI semantics: bigger frequency => slower evolution.
                double step = 0.20 / Math.max(0.05, freq);
                for (int i = 0; i < N; i++) {
                    double t = i * step;
                    double noise = 0.0;
                    double a = 1.0;
                    ...
                    for (int o = 0; o < oct; o++) {
                        double w = (1 << o);
                        noise += a * Math.sin(t * w + phase[o]);
                        sum += a;
                        a *= 0.5;
                    }
                    if (sum > 1e-6) noise /= sum;
                    double ms = baseDelay + ampDelay * noise;
                    y[i] = clampPreviewMs(ms);
                }
                break;
            }
            case SPManager.STREAM_NL_MODEL_PID_CONTROLLER: {
                double target = sp.getNlPidTargetSpeed();
                double p = sp.getNlPidP();
                double d = sp.getNlPidD();
                if (sPidTargetSpeed != null) target = sPidTargetSpeed.getValue();
                if (sPidP != null) p = sPidP.getValue();
                if (sPidD != null) d = sPidD.getValue();
                // Preview: PD controller converging towards a target delay.
                double targetDelay = 95.0 / Math.max(0.10, target);
                double cur = Math.max(12.0, targetDelay * 1.6);
                double prevErr = 0.0;
                for (int i = 0; i < N; i++) {
                    double err = targetDelay - cur;
                    double out = p * err + d * (err - prevErr);
                    cur = cur + out;
                    prevErr = err;
                    y[i] = clampPreviewMs(cur);
                }
                break;
            }
            case SPManager.STREAM_NL_MODEL_LOGISTIC_FATIGUE: {
                int burst = sp.getNlLogisticInitialBurstMs();
                double half = sp.getNlLogisticDecayHalfLife();
                double rec = sp.getNlLogisticRecovery();
                if (sLogiInitial != null) burst = Math.round(sLogiInitial.getValue());
                if (sLogiDecay != null) half = sLogiDecay.getValue();
                if (sLogiRecovery != null) rec = sLogiRecovery.getValue();
                // Preview: logistic fatigue that slows down over time and recovers on punctuation ticks.
                double halfLife = half;
                if (halfLife < 1.0) halfLife = 1.0;
                double decay = Math.log(2.0) / halfLife;

                double fatigue = 0.0;
                for (int i = 0; i < N; i++) {
                    fatigue = fatigue + (1.0 - fatigue) * decay;

                    double t0 = burst;
                    double t1 = Math.min(1800.0, t0 * 6.0 + 90.0);
                    double sCurve = 1.0 / (1.0 + Math.exp(-10.0 * (fatigue - 0.35)));
                    double ms = t0 + (t1 - t0) * sCurve;
                    y[i] = clampPreviewMs(ms);

                    // Apply recovery on our fixed punctuation visualization ticks.
                    if (rec > 0.0) {
                        for (int t : punctTicks) {
                            if (i == t) {
                                fatigue = Math.max(0.0, fatigue - rec * 0.45);
                                break;
                            }
                        }
                    }
            ...
            case SPManager.STREAM_NL_MODEL_RETRO_TYPEWRITER: {
                int key = sp.getNlRetroKeySpeedMs();
                int cr = sp.getNlRetroCrDelayMs();
                int drag = sp.getNlRetroPuncDragMs();
                if (sRetroKeySpeed != null) key = Math.round(sRetroKeySpeed.getValue());
                if (sRetroCrDelay != null) cr = Math.round(sRetroCrDelay.getValue());
                if (sRetroPuncDrag != null) drag = Math.round(sRetroPuncDrag.getValue());
                // Preview: base key speed + punctuation drag on punct ticks + one CR event mid-way.
                final int crTick = 52;
                for (int i = 0; i < N; i++) {
                    double ms = key;
                    if (i == crTick) ms += cr;
    ...
    private static String getModelDescription(@NonNull Context context, int model) {
        switch (model) {
            ...
            case SPManager.STREAM_NL_MODEL_PERLIN_NOISE:
                return context.getString(R.string.ui_streaming_nl_desc_perlin_noise);
            case SPManager.STREAM_NL_MODEL_PID_CONTROLLER:
                return context.getString(R.string.ui_streaming_nl_desc_pid_controller);
            case SPManager.STREAM_NL_MODEL_LOGISTIC_FATIGUE:
                return context.getString(R.string.ui_streaming_nl_desc_logistic_fatigue);
            case SPManager.STREAM_NL_MODEL_RETRO_TYPEWRITER:
                return context.getString(R.string.ui_streaming_nl_desc_retro_typewriter);
            default:
                return context.getString(R.string.ui_streaming_nl_desc_markov_random_walk);
        }
    }
    ...
    private static String buildSummary(@NonNull Context context,
        ...
        switch (model) {
            ...
            case SPManager.STREAM_NL_MODEL_PERLIN_NOISE: {
                ...
                if (sPerlinOctaves != null) oct = Math.round(sPerlinOctaves.getValue());
                if (sPerlinFrequency != null) freq = sPerlinFrequency.getValue();
                modelPart = String.format(Locale.US, "base=%sx, oct=%d, f=%s", formatDouble(base, 2), oct, formatDouble(freq, 2));
                break;
            }
            case SPManager.STREAM_NL_MODEL_PID_CONTROLLER: {
                double target = sp.getNlPidTargetSpeed();
                ...
                if (sPidP != null) p = sPidP.getValue();
                if (sPidD != null) d = sPidD.getValue();
                modelPart = String.format(Locale.US, "target=%sx, P=%s, D=%s", formatDouble(target, 2), formatDouble(p, 2), formatDouble(d, 2));
                break;
            }
            case SPManager.STREAM_NL_MODEL_LOGISTIC_FATIGUE: {
                int burst = sp.getNlLogisticInitialBurstMs();
                ...
                if (sLogiDecay != null) half = sLogiDecay.getValue();
                if (sLogiRecovery != null) rec = sLogiRecovery.getValue();
                modelPart = String.format(Locale.US, "burst=%dms, half=%s, rec=%s", burst, formatDouble(half, 0), formatDouble(rec, 2));
                break;
            }
            case SPManager.STREAM_NL_MODEL_RETRO_TYPEWRITER: {
                int key = sp.getNlRetroKeySpeedMs();
                ...
                if (sRetroCrDelay != null) cr = Math.round(sRetroCrDelay.getValue());
                if (sRetroPuncDrag != null) drag = Math.round(sRetroPuncDrag.getValue());
                modelPart = String.format(Locale.US, "key=%dms, CR=%dms, drag=%dms", key, cr, drag);
                break;
            }
            case SPManager.STREAM_NL_MODEL_MARKOV_RANDOM_WALK:
            default: {
    ...
    private static List<ModelRow> buildModelRows(@NonNull Context context) {
        ...
        rows.add(ModelRow.header(context.getString(R.string.ui_streaming_nl_group_geek)));
        rows.add(ModelRow.model(
                SPManager.STREAM_NL_MODEL_PERLIN_NOISE,
                context.getString(R.string.ui_streaming_nl_model_perlin_noise),
                context.getString(R.string.ui_streaming_nl_desc_perlin_noise)
        ));
        rows.add(ModelRow.model(
                SPManager.STREAM_NL_MODEL_PID_CONTROLLER,
                context.getString(R.string.ui_streaming_nl_model_pid_controller),
                context.getString(R.string.ui_streaming_nl_desc_pid_controller)
        ));
        rows.add(ModelRow.model(
                SPManager.STREAM_NL_MODEL_LOGISTIC_FATIGUE,
                context.getString(R.string.ui_streaming_nl_model_logistic_fatigue),
                context.getString(R.string.ui_streaming_nl_desc_logistic_fatigue)
        ));
        rows.add(ModelRow.model(
                SPManager.STREAM_NL_MODEL_RETRO_TYPEWRITER,
                context.getString(R.string.ui_streaming_nl_model_retro_typewriter),
                context.getString(R.string.ui_streaming_nl_desc_retro_typewriter)
        ));
        return rows;
    }
    ...
    private static final class TypingPreviewController {
        ...
        // v16+ geek models
        @Nullable private final Slider sPerlinBaseSpeed;
        @Nullable private final Slider sPerlinOctaves;
        @Nullable private final Slider sPerlinFrequency;

        @Nullable private final Slider sPidTargetSpeed;
        @Nullable private final Slider sPidP;
        @Nullable private final Slider sPidD;

        @Nullable private final Slider sLogiInitial;
        @Nullable private final Slider sLogiDecay;
        @Nullable private final Slider sLogiRecovery;

        @Nullable private final Slider sRetroKeySpeed;
        @Nullable private final Slider sRetroCrDelay;
        @Nullable private final Slider sRetroPuncDrag;
        ...
        private double pidCur = 0.0;

        // Logistic fatigue state
        private double logisticFatigue = 0.0;

        // Perlin state (multi-sine phases)
        ...
        TypingPreviewController(@NonNull Context context,
            ...
            this.sPerlinBaseSpeed = sPerlinBaseSpeed;
            this.sPerlinOctaves = sPerlinOctaves;
            this.sPerlinFrequency = sPerlinFrequency;

            this.sPidTargetSpeed = sPidTargetSpeed;
            this.sPidP = sPidP;
            this.sPidD = sPidD;

            this.sLogiInitial = sLogiInitial;
            this.sLogiDecay = sLogiDecay;
            this.sLogiRecovery = sLogiRecovery;

            this.sRetroKeySpeed = sRetroKeySpeed;
            this.sRetroCrDelay = sRetroCrDelay;
            this.sRetroPuncDrag = sRetroPuncDrag;
        ...
        private void start() {
            ...
            for (int i = 0; i < perlinPhaseOffsets.length; i++) {
                perlinPhaseOffsets[i] = rng.nextDouble() * (2.0 * Math.PI);
            }
        ...
        private long computeDelayMs(int n, char lastChar) {
            ...
            switch (model) {
                ...
                case SPManager.STREAM_NL_MODEL_PERLIN_NOISE: {
                    double baseSpeed = sp.getNlPerlinBaseSpeed();
                    int oct = sp.getNlPerlinOctaves();
                    double freq = sp.getNlPerlinFrequency();
                    if (sPerlinBaseSpeed != null) baseSpeed = sPerlinBaseSpeed.getValue();
                    if (sPerlinOctaves != null) oct = Math.round(sPerlinOctaves.getValue());
                    if (sPerlinFrequency != null) freq = sPerlinFrequency.getValue();
                    ...
                    double baseDelay = 95.0 / Math.max(0.10, baseSpeed);
                    double ampDelay = baseDelay * 0.35;

                    // Smooth multi-sine noise (Perlin-ish) with per-octave random phase offsets.
                    ...
                    double step = 0.20 / Math.max(0.05, freq); // smaller freq -> faster evolution
                    perlinPhase += step;
                ...
                case SPManager.STREAM_NL_MODEL_PID_CONTROLLER: {
                    double targetSpeed = sp.getNlPidTargetSpeed();
                    double p = sp.getNlPidP();
                    double d = sp.getNlPidD();
                    if (sPidTargetSpeed != null) targetSpeed = sPidTargetSpeed.getValue();
                    if (sPidP != null) p = sPidP.getValue();
                    if (sPidD != null) d = sPidD.getValue();

                    double targetDelay = 95.0 / Math.max(0.10, targetSpeed);

                    if (!pidInit) {
                        pidCur = Math.max(12.0, targetDelay * 1.6);
                        pidPrevErr = 0.0;
                        pidInit = true;
                    }

                    double err = targetDelay - pidCur;
                    double out = p * err + d * (err - pidPrevErr);
                    pidCur = pidCur + out;
                    pidPrevErr = err;
                ...
                case SPManager.STREAM_NL_MODEL_LOGISTIC_FATIGUE: {
                    int burst = sp.getNlLogisticInitialBurstMs();
                    double halfLife = sp.getNlLogisticDecayHalfLife();
                    double rec = sp.getNlLogisticRecovery();
                    if (sLogiInitial != null) burst = Math.round(sLogiInitial.getValue());
                    if (sLogiDecay != null) halfLife = sLogiDecay.getValue();
                    if (sLogiRecovery != null) rec = sLogiRecovery.getValue();

                    if (halfLife < 1.0) halfLife = 1.0;
                    double decay = Math.log(2.0) / halfLife;

                    // Fatigue builds up; recovery happens after pause chars.
                    ...
                    // Apply recovery AFTER computing this delay, so the rest affects the next ticks.
                    if (isPausePunct(lastChar) && rec > 0.0) {
                        logisticFatigue = Math.max(0.0, logisticFatigue - rec * 0.45);
                    }
                    break;
                }
                case SPManager.STREAM_NL_MODEL_RETRO_TYPEWRITER: {
