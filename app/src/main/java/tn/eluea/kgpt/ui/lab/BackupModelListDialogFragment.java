...
/**
 * 备用模型选择弹窗：交互与「AI 大模型」一致
 * - 列表高亮选中项
 * - 底部 取消/确定
 * - 未配置时拦截打开二级快速配置
 * - 支持“未设置”
 */
public class BackupModelListDialogFragment extends DialogFragment {

    public interface Callback {
        void onBackupModelConfirmed(@Nullable LanguageModel model);
    }

    public static final String TAG = "BackupModelListDialog";
    ...
    public static BackupModelListDialogFragment newInstance(@Nullable LanguageModel selected) {
        BackupModelListDialogFragment f = new BackupModelListDialogFragment();
        Bundle b = new Bundle();
        if (selected != null) b.putString(ARG_SELECTED, selected.name());
        f.setArguments(b);
        return f;
    }
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // Refresh list after quick config and allow auto-select.
        try {
            getParentFragmentManager().setFragmentResultListener(
                    ...
                    (requestKey, result) -> {
                        ...
                        try {
                            ...
                            boolean fetched = QuickProviderConfigDialogFragment.ACTION_FETCHED.equals(action)
                            ...
                            if (fetched) {
                                ...
                                if (name == null) name = result.getString("arg_provider", null);
                                if (name != null) {
                                    selected = LanguageModel.valueOf(name);
                                    selectedIsNotSet = false;
                                    if (adapter != null) adapter.setSelected(selected, false);
                                }
                            }
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        ...
        if (tvTitle != null) tvTitle.setText(R.string.ui_backup_model);
        ...
        try {
            ...
            if (args != null) {
                String s = args.getString(ARG_SELECTED, null);
                if (!TextUtils.isEmpty(s)) {
                    selected = LanguageModel.valueOf(s);
                    selectedIsNotSet = false;
                }
            }
        } catch (Throwable ignored) {}

        adapter = new Adapter(buildProviderList(), selected, selectedIsNotSet, new Adapter.Listener() {
            @Override
            public void onClickNotSet() {
                selected = null;
                selectedIsNotSet = true;
                adapter.setSelected(null, true);
            }
            ...
            public void onClickProvider(@NonNull LanguageModel provider) {
                ...
                selected = provider;
                selectedIsNotSet = false;
                adapter.setSelected(provider, false);
            }
        ...
        if (btnOk != null) {
            btnOk.setOnClickListener(v -> {
                ...
                if (cb != null) cb.onBackupModelConfirmed(selectedIsNotSet ? null : selected);
    ...
    private static final class Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        private final List<LanguageModel> providers;
        private final Listener listener;
        @Nullable private LanguageModel selected;
        private boolean selectedIsNotSet;

        Adapter(@NonNull List<LanguageModel> providers,
                @Nullable LanguageModel selected,
                boolean selectedIsNotSet,
...
private static int getApiKeyStateIcon(boolean hasKey, long lastCheck, boolean ok, @Nullable String err) {
    ...
    if (lastCheck > 0L && !ok) return R.drawable.ic_provider_state_expired;
...
private static String getApiKeyStateText(boolean hasKey, long lastCheck, boolean ok, @Nullable String err) {
    ...
    if (lastCheck > 0L && !ok) return isLikelyExpired(err) ? "APIKEY过期" : "APIKEY错误";
    return "APIKEY正常";
}
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
            ...
            final LanguageModel p = providers.get(position - 1);

            if (h.ivRefresh != null) {
                h.ivRefresh.setVisibility(View.VISIBLE);
            ...
            // Extra info (same as AI 大模型弹窗): Base URL host / last validation / latency & reachability.
            String baseHost = "-";
...
if (h.ivKey != null) {
    ...
    try { h.ivKey.setImageTintList(null); } catch (Throwable ignored) {}
            ...
            boolean isSel = (!selectedIsNotSet && selected == p);
            h.itemView.setBackgroundResource(isSel ? R.drawable.bg_list_item_ripple_selected : R.drawable.bg_list_item_ripple);
            h.itemView.setOnClickListener(v -> {
        ...
        public int getItemCount() {
            return (providers != null ? providers.size() : 0) + 1;
        }
