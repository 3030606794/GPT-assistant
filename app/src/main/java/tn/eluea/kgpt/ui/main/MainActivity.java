...
protected void attachBaseContext(android.content.Context newBase) {
    ...
    private FrameLayout navHome, navLab, navSettings;
    private ImageView navHomeIcon, navLabIcon, navSettingsIcon;
    ...
    /**
     * Migrates legacy dock indices (Home=0, Models=1, Lab=2, Settings=3)
     * into the current 3-tab dock (Home=0, Lab=1, Settings=2).
     */
    private int normalizeNavIndex(int legacyIndex) {
        if (legacyIndex <= 0) return 0;
        if (legacyIndex == 1) return 1; // Old "Models" -> Lab
        if (legacyIndex == 2) return 1; // Old Lab
        if (legacyIndex == 3) return 2; // Old Settings
        return 0;
    }
