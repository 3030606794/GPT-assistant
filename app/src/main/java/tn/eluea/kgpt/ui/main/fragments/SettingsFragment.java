...
import tn.eluea.kgpt.ui.settings.LanguageSelectionActivity;
...
public class SettingsFragment extends Fragment {
    ...
    private android.widget.ImageView iconLanguage;
    ...
    private void initViews(View view) {
        ...
        iconLanguage = view.findViewById(R.id.icon_language);
    ...
    private void applyAmoledIfNeeded() {
        ...
        boolean isDarkMode = PrefsCompat.getThemeIsDark(requireContext(), uiPrefs, PREF_THEME, false);
    ...
    private void setupListeners() {
        ...
        if (btnLanguage != null) {
            btnLanguage.setOnClickListener(v -> {
                Intent intent = new Intent(requireContext(), LanguageSelectionActivity.class);
...
private void updateCurrentLanguageLabel() {
    if (tvCurrentLanguage == null || !isAdded()) {
        return;
    }
    try {
        String code = LocaleHelper.getLanguage(requireContext());
        java.util.Locale locale = new java.util.Locale(code);
        String nativeName = locale.getDisplayName(locale);
        String englishName = locale.getDisplayName(java.util.Locale.ENGLISH);

        if (nativeName != null && nativeName.length() > 0) {
            nativeName = nativeName.substring(0, 1).toUpperCase(java.util.Locale.ROOT) + nativeName.substring(1);
        }
        if (englishName != null && englishName.length() > 0) {
            englishName = englishName.substring(0, 1).toUpperCase(java.util.Locale.ROOT) + englishName.substring(1);
        }

        if (englishName != null && !englishName.isEmpty()) {
            tvCurrentLanguage.setText(nativeName + " (" + englishName + ")");
        } else {
            tvCurrentLanguage.setText(nativeName);
        }
    } catch (Exception e) {
        // Fallback
        tvCurrentLanguage.setText(LocaleHelper.getLanguage(requireContext()));
    }
}
