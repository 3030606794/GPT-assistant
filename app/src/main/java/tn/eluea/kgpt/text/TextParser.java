...
public class TextParser implements ConfigChangeListener {
    ...
    private ParseResult adjustAiTriggerToCursorLineIfNeeded(
        ...
        try {
            ...
            if (sp == null) return original;

            int mode = sp.getAiTriggerMultilineMode();
            ParseResult result = original;

            // OFF => single-line behavior: pin to cursor line
            if (mode == SPManager.AI_TRIGGER_MULTILINE_OFF) {
                ...
                result = new AIParseResult(groups, lineStart, cursor, origAi.roleIdOverride, triggerSymbol);
            }

            // Apply newline policy for ON modes (keep / nl->space / remove)
            if (result instanceof AIParseResult) {
                AIParseResult ai = (AIParseResult) result;
                String p = ai.prompt;
                String p2 = sp.applyAiTriggerNewlinePolicy(p);
                if (p2 != null && !p2.equals(p)) {
                    java.util.ArrayList<String> groups2 = new java.util.ArrayList<>();
                    try {
                        if (ai.groups != null) groups2.addAll(ai.groups);
                    } catch (Throwable ignored) {}
                    while (groups2.size() <= 1) groups2.add("");
                    groups2.set(1, p2);
                    result = new AIParseResult(groups2, ai.indexStart, ai.indexEnd, ai.roleIdOverride, triggerSymbol);
                }
            }
    ...
    public ParseResult parse(String text, int cursor) {
            ...
            if (commandsEnabled && lastAskIndex >= 0) {
                ...
                for (ParseDirective directive : directives) {
                    ...
                    if (result != null) {
                        ...
                        // If this is a generic AI trigger without a role override, pin it to the currently selected role.
                        if (result instanceof AIParseResult) {
                            AIParseResult ai = (AIParseResult) result;
                            if (ai.roleIdOverride == null || ai.roleIdOverride.trim().isEmpty()) {
                                try {
                                    SPManager sp = SPManager.getInstance();
                                    if (sp != null) {
                                        String rid = sp.getActiveRoleId();
                                        if (rid != null && !rid.trim().isEmpty()) {
                                            result = new AIParseResult(ai.groups, ai.indexStart, ai.indexEnd, rid, currentTriggerSymbol);
                                        }
                                    }
                                } catch (Throwable ignored) {}
                            }
        ...
        for (ParseDirective directive : directives) {
            ...
            if (parseResult != null) {
                ...
                // If this is a generic AI trigger without a role override, pin it to the currently selected role.
                if (parseResult instanceof AIParseResult) {
                    AIParseResult ai = (AIParseResult) parseResult;
                    if (ai.roleIdOverride == null || ai.roleIdOverride.trim().isEmpty()) {
                        try {
                            SPManager sp = SPManager.getInstance();
                            if (sp != null) {
                                String rid = sp.getActiveRoleId();
                                if (rid != null && !rid.trim().isEmpty()) {
                                    parseResult = new AIParseResult(ai.groups, ai.indexStart, ai.indexEnd, rid, currentTriggerSymbol);
                                }
                            }
                        } catch (Throwable ignored) {}
                    }
