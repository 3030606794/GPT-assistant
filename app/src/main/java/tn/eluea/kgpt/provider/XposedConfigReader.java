...
public class XposedConfigReader {
    ...
    // Some IMEs (and thus the module) can start before the user unlocks after reboot.
    // In that phase CE storage and the LSPosed prefs file can be temporarily unreadable.
    // The previous logic would mark prefsAvailable=false permanently for the rest of the
    // boot session and never recover after unlock. We periodically retry initialization.
    private static long lastInitAttempt = 0;
    ...
    private static synchronized void initPrefs() {
        ...
        if (initialized) {
            // If previously unavailable, retry periodically (e.g. after unlock)
            if (!prefsAvailable && (now - lastInitAttempt) > INIT_RETRY_INTERVAL) {
                initialized = false;
            } else {
                return;
            }
    ...
    private static void reloadIfNeeded() {
        ...
        // If we were previously unavailable, try to re-init (after unlock)
        if (!prefsAvailable) {
            initPrefs();
    ...
    /**
     * Get long value from XSharedPreferences
     */
    public static long getLong(String key, long defaultValue) {
        ...
        try {
            long value = xPrefs.getLong(key, defaultValue);
            cache.put(key, value);
            Log.d(TAG, "getLong(" + key + ") = " + value);
            return value;
        } catch (Throwable e) {
            // Some LSPosed / ROM combos may not support getLong on older APIs; fall back to String parse
            try {
                String v = xPrefs.getString(key, null);
                if (v != null) {
    ...
    /**
     * Get float value from XSharedPreferences
     */
    public static float getFloat(String key, float defaultValue) {
        ...
        try {
            float value = xPrefs.getFloat(key, defaultValue);
            cache.put(key, value);
            return value;
        } catch (Exception e) {
            Log.e(TAG, "Failed to read float: " + key, e);
        }
