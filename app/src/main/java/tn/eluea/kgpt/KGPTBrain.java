...
public class KGPTBrain implements InputEventListener, DialogDismissListener {
    ...
    public void onTextUpdate(String text, int cursor) {
        ...
        if (result != null) {
            // Some editors keep a trailing newline/whitespace in extracted text.
            // Java regex '$' can match *before* a final newline, making indexEnd < cursor.
            // For word-based triggers (e.g., 模型切换 / AI剪贴板), allow trailing whitespace so they still fire.
            int effectiveEnd = result.indexEnd;
            boolean allowTrailingWs = (result instanceof tn.eluea.kgpt.text.parse.result.SettingsParseResult)
                    || (result instanceof tn.eluea.kgpt.text.parse.result.ModelSwitchParseResult)
                    || (result instanceof tn.eluea.kgpt.text.parse.result.AIClipboardParseResult)
                    || (result instanceof tn.eluea.kgpt.text.parse.result.QuickJumpMenuParseResult);
            if (allowTrailingWs && effectiveEnd < cursor) {
                try {
                    String tail = text.substring(effectiveEnd, cursor);
            ...
            // Guard: prevent accidental popups (model/config panels) triggered by composing previews
            // or stale selection updates. Only allow UI-type triggers shortly after a non-composing
            // editor mutation (commit/delete/replace).
            boolean isUiTrigger = (result instanceof tn.eluea.kgpt.text.parse.result.SettingsParseResult)
                    || (result instanceof tn.eluea.kgpt.text.parse.result.ModelSwitchParseResult)
                    || (result instanceof tn.eluea.kgpt.text.parse.result.AIClipboardParseResult)
                    || (result instanceof tn.eluea.kgpt.text.parse.result.QuickJumpMenuParseResult);
            if (isUiTrigger) {
                final long UI_TRIGGER_WINDOW_MS = 12000; // 12s
                if (imsController.isComposingActive() || !imsController.isRecentNonComposingMutation(UI_TRIGGER_WINDOW_MS)) {
                    return;
                }
            }
            ...
            int deleteCount = effectiveEnd - result.indexStart;
                ...
                // IMPORTANT:
                // We temporarily silence input event mirroring while deleting the trigger text.
                // Some editors occasionally throw during delete operations; if that happens and we
                // don't restore notify state, future triggers stop working ("second trigger" bug).
                imsController.stopNotifyInput();
                try {
                    boolean isAiSingleLine =
                            (result instanceof AIParseResult)
                                    && !SPManager.getInstance().getAiTriggerMultilineEnabled();

                    if (isAiSingleLine) {
                        // Delete only the current cursor line (without touching the previous line)
                        // Use absolute range deletion first (safer in some editors), and fall back to relative delete.
                        int cur = result.indexEnd;
                        int lineEnd = text.length();
                        int n1 = text.indexOf('\n', cur);
                        int n2 = text.indexOf('\r', cur);
                        int next;
                        if (n1 >= 0 && n2 >= 0) next = Math.min(n1, n2);
                        else next = Math.max(n1, n2);
                        if (next >= 0) lineEnd = next;

                        boolean ok = false;
                        try {
                            ok = imsController.deleteRange(result.indexStart, lineEnd);
                        } catch (Throwable ignored) {}

                        if (!ok) {
                            int after = Math.max(0, lineEnd - cur);
                            try {
                                imsController.deleteSurrounding(deleteCount, after);
                            } catch (Throwable ignored) {}
                        }
                    } else {
                        try {
