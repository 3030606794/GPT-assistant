...
public abstract class LanguageModelClient {
    ...
    /** Internal marker emitted by stream parser to keep watchdog alive; filtered before UI insertion. */
    public static final String INTERNAL_KEEPALIVE_MARKER = "__KGPT_INTERNAL_KEEPALIVE__";
...
private Map<LanguageModelField, String> mFields = new HashMap<>();
    ...
    public String getApiKey() {
        // API keys should never contain whitespace. Users sometimes paste keys with
        // trailing spaces/newlines; HttpURLConnection may silently drop such headers,
        // resulting in server-side "Missing Authentication header" errors.
        String key = getField(LanguageModelField.ApiKey);
        if (key == null) return null;
        // Trim + remove ALL whitespace characters (space/newline/tab/etc).
