...
public final class TopStatusBanner {
    ...
    private CircularProgressIndicator tokenBurner;
    private boolean added;

    // Token burner state (applied when the view is inflated/visible)
    private boolean burnerWanted;
    private int burnerProgress; // 0..100
    private int burnerColor = 0xFF4CAF50; // default green
    ...
    /** Update banner text and the small token burner indicator (safe from any thread). */
    public void updateState(String msg, boolean showBurner, int progress0to100, int indicatorColor) {
        burnerWanted = showBurner;
        burnerProgress = clamp(progress0to100, 0, 100);
        burnerColor = indicatorColor;
        main.post(() -> {
            if (text != null) {
                text.setText(msg == null ? "" : msg);
            }
    ...
    private void showInternal(Context app, String msg) {
        ...
        if (root == null) {
            ...
            tokenBurner = root.findViewById(R.id.kgpt_top_banner_token_burner);
            root.setAlpha(0f);
        }
        if (text != null) {
            text.setText(msg == null ? "" : msg);
    ...
    private void applyBurnerState() {
        if (tokenBurner == null) return;
        try {
            tokenBurner.setIndeterminate(false);
            tokenBurner.setMax(100);
            tokenBurner.setProgress(burnerProgress);
            tokenBurner.setIndicatorColor(burnerColor);
            tokenBurner.setVisibility(burnerWanted ? View.VISIBLE : View.GONE);
        } catch (Throwable ignored) {
        }
    }
